...
@testset "broadcast[!] over combinations of scalars, structured matrices, and dense vectors/matrices" begin
    ...
    mul2(X) = (f(x) = 2.0x; g(x) = f.(X); @inferred(g(X)))
    mult2(X) = X .* (2.0,) # constprop failed as `copyto!` is not inlined
    mulinf(X) = (f(x) = x*Inf; g(x) = f.(X); @inferred(g(X)))
    lpow2(X) = (g(X) = X .^ 2; @inferred(g(X)))
    lpow0(X) = (g(X) = X .^ 0; @inferred(g(X)))
    pow2(X) = (f(x) = (two = 2; x^two); g(x) = f.(X); @inferred(g(X)))
    lpow_1(X) = (g(X) = X .^ -1; @inferred(g(X)))
    powt2(X) = X .^ (2,) # constprop failed as `copyto!` is not inlined
