...
{
    r = r - e->offset;
    if (e->rec_len && r > 0 && ((9 * (r-1) + 8) < e->rec_len)){
        if (round) {
            e->envout[9 * (r-1) + 1] = jl_box_long(e->extra_flag);
            e->envout[9 * (r-1) + 3] = jl_box_long(loc);
            e->envout[9 * (r-1) + 6] = res;
            e->envout[9 * (r-1) + 8] = cache_envb(e);
        }
        else {
            e->envout[9 * (r-1) + 0] = jl_box_long(r + e->offset);
            e->envout[9 * (r-1) + 2] = jl_box_long(id);
            e->envout[9 * (r-1) + 4] = x;
            e->envout[9 * (r-1) + 5] = y;
            e->envout[9 * (r-1) + 7] = cache_envb(e);
        }
    }
}
...
static int var_lt(jl_tvar_t *b, jl_value_t *a, jl_stenv_t *e, int param)
{
    // if (e->rec_len && e->extra_flag > MAXNUM + e->rec_ind)
    //     return 0;
    e->extra_flag++;
    int r = e->extra_flag;
    int loc = 0;
...
static int has_right_var_in_union(jl_value_t *x, jl_stenv_t *e, jl_typeenv_t *log) JL_NOTSAFEPOINT
{
    if (jl_is_uniontype(x)) {
        return has_right_var_in_union(((jl_uniontype_t *)x)->a, e, log) ||
               has_right_var_in_union(((jl_uniontype_t *)x)->b, e, log);
    }
    else if (jl_is_unionall(x)) {
        jl_typeenv_t newlog = { ((jl_unionall_t *)x)->var, NULL, log };
        return has_right_var_in_union(((jl_unionall_t *)x)->body, e, &newlog);
    }
...
static int _subtype(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, int param, int *loc)
{
    ...
    if (jl_is_typevar(x)) {
        ...
        // jl_varbinding_t *xx = lookup(e, (jl_tvar_t*)x);
        // if ((xx == NULL || !xx->right) && jl_is_unionall(y) && has_right_var_in_union(y, e, NULL)) {
        //     *loc = 4017;
...
static int reachable_var(jl_value_t *x, jl_tvar_t *y, jl_stenv_t *e)
{
    ...
    // int res =
    // if (e->rec_len) {
    //     push_rec(16, 0, x, (jl_value_t *)y, NULL, r, 0, e);
    //     push_rec(16, 0, x, (jl_value_t *)y, jl_box_long(res), r, 1, e);
    // }
...
JL_DLLEXPORT jl_value_t *jl_intersect_toy(jl_value_t *x, jl_value_t *y, jl_value_t **env, int envsz, int offset)
{
    jl_stenv_t e;
    init_stenv(&e, env, 0);
    e.rec_len = envsz;
...
JL_DLLEXPORT int jl_subtype_env_toy(jl_value_t *x, jl_value_t *y, jl_value_t **env, int envsz, int offset)
{
    jl_stenv_t e;
    init_stenv(&e, env, 0);
    e.rec_len = envsz;
